...
wchar_t const *pDpcppRemappingV8Extract = _T("DpcppRemappingV8 Extract Kernel");
__itt_string_handle *handle_DpcppRemappingV8_extract_kernel = __itt_string_handle_create(pDpcppRemappingV8Extract);
wchar_t const *pDpcppRemappingV8Calc = _T("DpcppRemappingV8 Calc Kernel");
__itt_string_handle *handle_DpcppRemappingV8_calc_kernel = __itt_string_handle_create(pDpcppRemappingV8Calc);
#endif

DpcppRemappingV8::DpcppRemappingV8(SParameters& parameters) : DpcppBaseAlgorithm(parameters)
{
	m_storageType = STORAGE_TYPE_USM;
}

std::string DpcppRemappingV8::GetDescription()
{
	...
	case STORAGE_TYPE_USM:
		return "DpcppRemappingV8: V1 except uses fp32 instead of fp64 on " + strDesc;
		break;
	}
...
void DpcppRemappingV8::FrameCalculations(bool bParametersChanged)
{
	if (bParametersChanged || m_bFrameCalcRequired)
	{
#ifdef VTUNE_API
		__itt_task_begin(pittTests_domain, __itt_null, __itt_null, handle_DpcppRemappingV8_calc_kernel);
#endif
		...
		ComputeXYZCoords();
		TimingStats::GetTimingStats()->AddIterationResults(ETimingType::TIMING_CREATE_XYZ_COORDS, m_pParameters->m_uiDevIndex, startTime, std::chrono::high_resolution_clock::now());

		startTime = std::chrono::high_resolution_clock::now();
		ComputeLonLatCoords();
		TimingStats::GetTimingStats()->AddIterationResults(ETimingType::TIMING_CREATE_LON_LAT_COORDS, m_pParameters->m_uiDevIndex, startTime, std::chrono::high_resolution_clock::now());

		startTime = std::chrono::high_resolution_clock::now();
		ComputeXYCoords();
		TimingStats::GetTimingStats()->AddIterationResults(ETimingType::TIMING_CREATE_XY_COORDS, m_pParameters->m_uiDevIndex, startTime, std::chrono::high_resolution_clock::now());
...
cv::Mat DpcppRemappingV8::ExtractFrameImage()
{
#ifdef VTUNE_API
	__itt_task_begin(pittTests_domain, __itt_null, __itt_null, handle_DpcppRemappingV8_extract_kernel);
#endif
...
void DpcppRemappingV8::ComputeXYZCoords()
{
	float f;
	float cx;
	float cy;
	...
	f = 0.5f * m_pParameters->m_widthOutput / tan(0.5 * m_pParameters->m_fov / 180.0f * M_PI);
...
void DpcppRemappingV8::ComputeRotationMatrix(float radTheta, float radPhi, float radPsi)
{
	...
	//R = Rz @ Rx @ Ry
	cv::Mat x_axis = (cv::Mat_<float>(3, 1) << 1.0f, 0.0f, 0.0f);
	cv::Mat y_axis = (cv::Mat_<float>(3, 1) << 0.0f, 1.0f, 0.0f);
	cv::Mat z_axis = (cv::Mat_<float>(3, 1) << 0.0f, 0.0f, 1.0f);
	cv::Mat Rx;
	cv::Mat Ry;
	cv::Mat Rz;
	cv::Mat R;
...
void DpcppRemappingV8::ConvertXYZToLonLat()
{
...
void DpcppRemappingV8::ComputeLonLatCoords()
{
...
void DpcppRemappingV8::ComputeXYCoords()
{
	...
	case STORAGE_TYPE_USM:
	{
		m_pQ->submit([&](sycl::handler& cgh) {
			...
			[=](sycl::id<2> item) {
				...
				pElement->m_x = (pLonLatElement->m_x / xDiv + 0.5f) * imageWidth;
				pElement->m_y = (pLonLatElement->m_y / M_PI + 0.5f) * imageHeight;
			});
		});
		m_pQ->wait();
...
bool DpcppRemappingV8::StartVariant()
{
	...
	if (m_pQ)
	{
		...
		case STORAGE_TYPE_USM:
			...
			printf("DpcppRemappingV8::StartVariant STORAGE_TYPE_USM\n");
			break;
		}
	}
...
void DpcppRemappingV8::StopVariant()
{
	auto ctxt = m_pQ->get_context();
