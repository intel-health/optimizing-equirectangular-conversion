...
std::string SerialRemappingV1b::GetDescription()
{
	...
	{
	case STORE_ROW_COL:
		return "V1b = Changed V1a to access m_pXYZPoints by moving a pointer instead of array indexing each time.  Memory array of structure row/column layout.";
		break;
	case STORE_COL_ROW:
		return "V1b = Changed V1a to access m_pXYZPoints by moving a pointer instead of array indexing each time.  Memory array of structure column/row layout.";
		break;
	}
