...
void DpcppRemappingV8::ComputeXYZCoords()
{
	...
	f = 0.5f * m_parameters->m_widthOutput / tan(0.5 * m_parameters->m_fov / 180.0f * M_PI);
...
void DpcppRemappingV8::ComputeXYCoords()
{
	...
	float imageWidth = m_parameters->m_image[m_parameters->m_imageIndex].cols - 1.0f;
	float imageHeight = m_parameters->m_image[m_parameters->m_imageIndex].rows - 1.0f;
	int height = m_parameters->m_heightOutput;
	int width = m_parameters->m_widthOutput;
